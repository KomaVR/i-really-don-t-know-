        ...
        if (aiPrompt) {
            ...
            try {
                if (name === 'catfact') {
                    ...
                    const song = songOption || prompt; // fallback to previous behavior
                    const artist = artistOption;

                    if (!song) {
                        embed.description = '❌ Please provide a song name. Example: `/lyrics song:"Shape of You" artist:"Ed Sheeran"`';
                    } else {
                        if (!GENIUS_TOKEN) {
                            embed.title = 'Error';
                            ...
                            try {
                                ...
                                if (status === 401 || gRes.status === 401) {
                                    ...
                                    if (hits.length === 0) {
                                        ...
                                        embed.title = `${songResult.title} — ${songResult.primary_artist.name}`;
                                        embed.description = `[Click here to view the full lyrics on Genius](${songResult.url})`;
                                        // Use thumbnail if available
                                        if (songResult.song_art_image_thumbnail_url) {
                                            embed.thumbnail = {
                                                url: songResult.song_art_image_thumbnail_url
                                            };
                                        } else if (songResult.song_art_image_url) {
                                            embed.image = {
